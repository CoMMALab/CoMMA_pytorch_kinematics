...
def compute_pose_error(goal_tf, end_effector_tf):
    ...
    goal_rot = pk.matrix_to_euler_angles(goal_tf.get_matrix()[..., :3, :3], "XYZ")
    end_effector_rot = pk.matrix_to_euler_angles(end_effector_tf.get_matrix()[..., :3, :3], "XYZ")
...
def test_single_robot_ik_visualize(robot="kuka_iiwa", num_retries=10, max_iterations = 10, skip=False):
    ...
    for ii in range(num_retries):
        ...
        end_effector_tf = rob_tf.compose(end_effector_tf)
        
        # Compute errors
        pos_error, rot_error = compute_pose_error(goal_tf, end_effector_tf)
        ...
        write_to_csv(pos_error.item(), rot_error.item(), 1, max_iterations, "no_interpolation.csv")
...
def write_to_csv(pos_error, rot_error, n, max_iterations, csv_filename):
    file_exists = os.path.isfile(csv_filename)

    with open(csv_filename, mode='a', newline='') as csv_file:
        fieldnames = ['Position_Error', 'Rotation_Error', 'n', 'Max_Iterations']
...
def write_to_csv_with_step(pos_error, rot_error, n, max_iterations, step_number, csv_filename):
    file_exists = os.path.isfile(csv_filename)

    with open(csv_filename, mode='a', newline='') as csv_file:
        fieldnames = ['Step_Number', 'Position_Error', 'Rotation_Error', 'n', 'Max_Iterations']
        writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
...
def interpolate_poses(start_transform, end_transform, n):
    device = start_transform.device

    # Extract start and end transformation matrices
    start_matrix = start_transform.get_matrix()
    end_matrix = end_transform.get_matrix()

    # Extract position (last column) and rotation (upper 3x3 part) from the transformation matrices
    start_pos = start_matrix[..., :3, 3]
    end_pos = end_matrix[..., :3, 3]

    start_rot = pk.matrix_to_quaternion(start_matrix[..., :3, :3])
    end_rot = pk.matrix_to_quaternion(end_matrix[..., :3, :3])

    interpolated_poses = []

    # Interpolation steps
    for t in torch.linspace(0, 1, n, device=device):
        # LERP for position
        interp_pos = (1 - t) * start_pos + t * end_pos

        # SLERP for quaternion rotation
        interp_rot = pk.slerp(start_rot, end_rot, t)

        # Reconstruct the rotation matrix from the interpolated quaternion
        interp_rot_matrix = pk.quaternion_to_matrix(interp_rot)
        ...
        # Create an interpolated Transform3d from the matrix
        interp_transform = pk.Transform3d(matrix=interp_matrix)
        interpolated_poses.append(interp_transform)
...
def test_single_robot_ik_visualize_interpolation(robot="kuka_iiwa", num_retries=10, max_iterations=10, skip=False, n=10):
    ...
    interpolated_tfs = interpolate_poses(start_tf, end_tf, n)
   ...
   # Transform to world frame for visualization
    ...
    for ii in range(num_retries):
        ...
        for dof in range(len(original_joint_states)):
                p.resetJointState(armId, dof, original_joint_states[dof])
        for step, solutions in enumerate(sol):
            ...
            # Compute errors
            pos_error, rot_error = compute_pose_error(all_tfs[step], end_effector_tf)
...
if __name__ == "__main__":
    ...
    #         for n in range(1,11):
    # test_single_robot_ik_visualize(robot="widowx", num_retries=100, max_iterations=mi)
    mi = 10
    n=10
    test_single_robot_ik_visualize_interpolation(robot="widowx", num_retries=5, max_iterations=mi, skip=True,n=n)