...
from python_robotics_middleware import transforms
...
class Jacobian:
    ...
    def jacobian(self, serial_chain, th, locations=None, tool=None, ret_eef_pose=False):
        """
        ...
        """
        if isinstance(serial_chain, SerialChain):
            if locations is not None:
                    tool = transforms.Transform3d(pos=locations)
        ...
        # TODO: Fix to not require forward_kinematics
        pose = fk.forward_kinematics(serial_chain, th).get_matrix()
        rotation = pose[:, :3, :3]
